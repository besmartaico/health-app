...
function cleanReferredBy(val) { if(!val)return''; if(/^\$?[\d,]+\.?\d*$/.test(val.trim()))return''; return val; }
...
async function decrementInventory(sheets,lines) {
  try {
    const invRes=await sheets.spreadsheets.values.get({spreadsheetId:SID(),range:'Inventory!A2:N'});
    const rows=invRes.data.values||[];
    for(const line of lines){
      const idx=rows.findIndex(r=>r[1]?.toLowerCase().trim()===line.product?.toLowerCase().trim());
      if(idx>=0&&(rows[idx][12]||'').toLowerCase()!=='true'){
        // Col D (index 3) is quantity
        const curQty=parseFloat(rows[idx][3])||0;
        const newQty=Math.max(0,curQty-(parseFloat(line.qty)||1));
        await sheets.spreadsheets.values.update({spreadsheetId:SID(),range:`Inventory!D${idx+2}`,valueInputOption:'RAW',requestBody:{values:[[String(newQty)]]}});
      }
    }
...
async function addReferralCredit(sheets,referrerName){
  try{
    ...
    if(idx>=0){const newCredit=(parseFloat(rows[idx][9])||0)+20;await sheets.spreadsheets.values.update({spreadsheetId:SID(),range:`Customers!J${idx+2}`,valueInputOption:'RAW',requestBody:{values:[[String(newCredit)]]}});}
