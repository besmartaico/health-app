...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const handleSubmit = async () => {
    ...
    if (!email.trim()) {
      // No email = PIN login
      const res = await fetch('/api/auth', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ pin: password }) });
      const d = await res.json();
      if (d.success) { sessionStorage.setItem('admin_auth','true'); setAuth(true); }
      else setError('Incorrect PIN or credentials');
    } else {
      // Email provided = user login
  ...
  if (!auth) return (
    <div style={{minHeight:'100vh',background:'#0a0a0a',display:'flex',alignItems:'center',justifyContent:'center',padding:'24px'}}>
      <div style={{background:'#111',border:'1px solid #1f1f1f',borderRadius:'20px',padding:'40px',width:'100%',maxWidth:'380px',boxShadow:'0 32px 64px rgba(0,0,0,0.5)'}}>
        <div style={{textAlign:'center',marginBottom:'32px'}}>
          ...
          <p style={{color:'#4b5563',fontSize:'12px',margin:'6px 0 0'}}>Enter your email + password, or leave email blank and enter your PIN</p>
        </div>
        ...
        <div style={{marginBottom:'12px'}}>
          <input type='email' placeholder='Email (optional for PIN login)' value={email} onChange={e=>setEmail(e.target.value)} onKeyDown={e=>e.key==='Enter'&&handleSubmit()} style={inp} autoFocus />
        </div>
        <div style={{marginBottom:'20px',position:'relative'}}>
          <input type={showPw?'text':'password'} placeholder={email?'Password':'PIN'} value={password} onChange={e=>setPassword(e.target.value)} onKeyDown={e=>e.key==='Enter'&&handleSubmit()} style={{...inp,paddingRight:'44px'}} />
          <button onClick={()=>setShowPw(v=>!v)} style={{position:'absolute',right:'12px',top:'50%',transform:'translateY(-50%)',background:'transparent',border:'none',color:'#6b7280',cursor:'pointer',fontSize:'16px'}}>{showPw?'🙈':'👁'}</button>
        </div>
        <button onClick={handleSubmit} disabled={loading||!password} style={{width:'100%',background:password&&!loading?'#7b1c2e':'#2d0e18',color:password&&!loading?'#fff':'#5a2030',border:'none',borderRadius:'10px',padding:'13px',fontSize:'14px',fontWeight:700,cursor:password&&!loading?'pointer':'not-allowed',marginBottom:'16px'}}>{loading?'Signing in...':'Sign In'}</button>
        <div style={{textAlign:'center'}}>
          <a href='/signup' style={{color:'#4b5563',fontSize:'12px',textDecoration:'none'}} onMouseOver={e=>e.currentTarget.style.color='#9ca3af'} onMouseOut={e=>e.currentTarget.style.color='#4b5563'}>Create account with invite link →</a>
        </div>
      </div>
    </div>
  );
