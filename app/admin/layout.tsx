    ...
    <div style={{display:'flex',minHeight:'100vh',background:'#131313',fontFamily:'Inter,system-ui,sans-serif'}}>
      <aside style={{width:'220px',background:'#0f0f0f',display:'flex',flexDirection:'column',flexShrink:0,position:'sticky',top:0,height:'100vh',overflowY:'auto',borderRight:'1px solid #1f1f1f'}}>
        ...
        <nav style={{padding:'10px 8px',flex:1}}>
          {NAV.map(({label,href,icon})=>{ const active=path===href||(href!=='/admin'&&path.startsWith(href)); return(<a key={href} href={href} onClick={()=>setPath(href)} style={{display:'flex',alignItems:'center',gap:'10px',padding:'8px 10px',borderRadius:'8px',textDecoration:'none',marginBottom:'1px',fontSize:'13px',fontWeight:active?600:400,background:active?'rgba(123,28,46,0.25)':'transparent',color:active?'#f87171':'#9ca3af',borderLeft:active?'2px solid #7b1c2e':'2px solid transparent'}} onMouseOver={e=>{if(!active){e.currentTarget.style.background='rgba(255,255,255,0.04)';e.currentTarget.style.color='#e5e7eb';}}} onMouseOut={e=>{if(!active){e.currentTarget.style.background='transparent';e.currentTarget.style.color='#9ca3af';}}}><span style={{fontSize:'13px',width:'18px',textAlign:'center'}}>{icon}</span>{label}</a>);
          })}
          <div style={{borderTop:'1px solid #1f1f1f',margin:'8px 2px',paddingTop:'8px'}}>
            <a href='/api/sso' target='_blank' rel='noreferrer' style={{display:'flex',alignItems:'center',gap:'10px',padding:'8px 10px',borderRadius:'8px',textDecoration:'none',fontSize:'13px',fontWeight:400,background:'transparent',color:'#9ca3af',borderLeft:'2px solid transparent'}} onMouseOver={e=>{e.currentTarget.style.background='rgba(59,130,246,0.08)';e.currentTarget.style.color='#60a5fa';e.currentTarget.style.borderLeftColor='rgba(59,130,246,0.4)';}} onMouseOut={e=>{e.currentTarget.style.background='transparent';e.currentTarget.style.color='#9ca3af';e.currentTarget.style.borderLeftColor='transparent';}}>
              <span style={{fontSize:'13px',width:'18px',textAlign:'center'}}>📝</span>Documents
              <span style={{marginLeft:'auto',fontSize:'10px',color:'#374151'}}>↗</span>
            </a>
          </div>
        </nav>
