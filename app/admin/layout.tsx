      ...
      <div style={{ width: '100%', maxWidth: '400px' }}>
        ...
        <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '20px', padding: '36px' }}>
          ...
          <button onClick={login} disabled={loading || !pin}
            style={{ width: '100%', background: pin && !loading ? '#7b1c2e' : '#2d0e18', color: pin && !loading ? '#fff' : '#6b2d3e', border: 'none', borderRadius: '10px', padding: '15px', fontSize: '16px', fontWeight: '700', cursor: pin && !loading ? 'pointer' : 'not-allowed', marginTop: '16px', transition: 'all 0.2s' }}
          ...
          <p style={{ color: '#374151', fontSize: '11px', textAlign: 'center', marginTop: '20px', marginBottom: 0, lineHeight: '1.6' }}>Unauthorized access attempts are logged.</p>
        </div>
      </div>
    </div>
  );
          ...
          {NAV.map(({ label, href, icon }) => {
            ...
            return (
              <a key={href} href={href} onClick={() => setCurrentPath(href)}
                style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '9px 10px', borderRadius: '8px', textDecoration: 'none', marginBottom: '2px', fontSize: '13px', fontWeight: active ? '600' : '400', background: active ? 'rgba(123,28,46,0.25)' : 'transparent', color: active ? '#f87171' : '#9ca3af', borderLeft: active ? '2px solid #7b1c2e' : '2px solid transparent', transition: 'all 0.15s' }}
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'auto' }}>
        ...
        <div style={{ background: '#0f0f0f', borderBottom: '1px solid #1f1f1f', padding: '0 28px', height: '56px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 10, flexShrink: 0 }}>
          <div style={{ color: '#6b7280', fontSize: '13px' }}>
            Welcome back, <span style={{ color: '#c0394f', fontWeight: '700' }}>Admin</span>
          </div>
        ...
        {/* Page */}
        <div style={{ flex: 1, overflow: 'auto', background: '#131313' }}>
          {children}
        </div>
      </div>
    </div>
