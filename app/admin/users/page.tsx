...
const ROLES = {
  owner: { label: 'Owner', bg: 'rgba(139,92,246,0.15)', color: '#a78bfa', border: 'rgba(139,92,246,0.3)' },
  admin: { label: 'Admin', bg: 'rgba(16,185,129,0.12)', color: '#34d399', border: 'rgba(16,185,129,0.25)' },
  viewer: { label: 'Viewer', bg: 'rgba(59,130,246,0.12)', color: '#60a5fa', border: 'rgba(59,130,246,0.25)' },
};

const RoleBadge = ({ role }) => {
  const r = ROLES[role] || ROLES.viewer;
  return <span style={{ background: r.bg, color: r.color, border: '1px solid ' + r.border, borderRadius: '20px', padding: '3px 10px', fontSize: '11px', fontWeight: 600 }}>{r.label}</span>;
};
...
export default function UsersPage() {
  ...
  const [form, setForm] = useState({ email: '', name: '', role: 'admin', teamId: '' });
  ...
  const load = async () => {
    ...
    try {
      ...
      setUsers(ur.users || []);
      setInvites((ur.invites || []).filter(i => i.status !== 'revoked'));
      setTeams(tr.teams || []);
    } catch {}
    setLoading(false);
  };
  ...
  const sendInvite = async () => {
    ...
    try {
      ...
      setForm({ email: '', name: '', role: 'admin', teamId: '' });
      setShowInvite(false);
      await load();
  ...
  const updateTeam = async (email, teamId) => {
    ...
    <div style={{ background: '#131313', minHeight: '100vh', padding: '28px', maxWidth: '1000px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '28px' }}>
        <div>
          ...
          <p style={{ color: '#6b7280', fontSize: '13px', margin: 0 }}>Manage access and team assignments</p>
        </div>
        <div style={{ display: 'flex', gap: '8px' }}>
          <a href="/admin/teams" style={{ background: '#1e1e1e', color: '#9ca3af', border: '1px solid #2a2a2a', borderRadius: '9px', padding: '10px 16px', fontSize: '13px', fontWeight: 600, textDecoration: 'none' }}>
            🏢 Manage Teams
          </a>
        ...
        {[['owner', 'Owner', 'Sees all teams'], ['admin', 'Admin', 'Team-scoped access'], ['viewer', 'Viewer', 'Read-only, team-scoped']].map(([role, label, desc]) => (
          <div key={role} style={{ background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: '12px', padding: '16px 18px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
              <RoleBadge role={role} />
              <span style={{ color: '#4b5563', fontSize: '12px' }}>{countByRole(role)}</span>
            </div>
      ...
      <div style={{ display: 'flex', gap: '4px', background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: '10px', padding: '4px', marginBottom: '20px', width: 'fit-content' }}>
        {[['users', 'Active Users', users.length], ['invites', 'Pending Invites', invites.length]].map(([tab, label, count]) => (
          <button key={tab} onClick={() => setActiveTab(tab)} style={{ background: activeTab === tab ? '#0f0f0f' : 'transparent', color: activeTab === tab ? '#fff' : '#6b7280', border: activeTab === tab ? '1px solid #2a2a2a' : '1px solid transparent', borderRadius: '7px', padding: '7px 16px', fontSize: '13px', fontWeight: activeTab === tab ? 600 : 400, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '7px' }}>
            {label}
            <span style={{ background: activeTab === tab ? '#7b1c2e' : '#222', color: activeTab === tab ? '#fff' : '#4b5563', borderRadius: '20px', padding: '1px 7px', fontSize: '11px', fontWeight: 700 }}>{count}</span>
          </button>
        ))}
      </div>
        ...
        <div style={{ background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: '14px', overflow: 'hidden' }}>
          {loading ? (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>{['User', 'Role', 'Team', 'Added', ''].map(h => <th key={h} style={th}>{h}</th>)}</tr>
              </thead>
              <tbody>
                {users.map(u => (
                  <tr key={u.email} onMouseOver={e => e.currentTarget.style.background = '#1f1f1f'} onMouseOut={e => e.currentTarget.style.background = 'transparent'}>
                    ...
                    <td style={td}>
                      {u.role === 'owner' ? (
                        <span style={{ color: '#374151', fontSize: '12px', fontStyle: 'italic' }}>All teams</span>
                      ) : teams.length === 0 ? (
                        <a href="/admin/teams" style={{ color: '#c0394f', fontSize: '12px', textDecoration: 'none' }}>Create teams →</a>
                      ) : (
      ...
      {activeTab === 'invites' && (
        <div style={{ background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: '14px', overflow: 'hidden' }}>
          {invites.length === 0 ? (
            <div style={{ padding: '48px', textAlign: 'center' }}>
              <div style={{ fontSize: '32px', marginBottom: '12px' }}>📬</div>
              <div style={{ color: '#fff', fontWeight: 600 }}>No pending invites</div>
            </div>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <div style={{ background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: '18px', padding: '32px', width: '100%', maxWidth: '440px', boxShadow: '0 32px 64px rgba(0,0,0,0.6)' }}>
            ...
            <div style={{ marginBottom: '16px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {[['admin', 'Admin', 'Full portal access'], ['viewer', 'Viewer', 'Read-only access']].map(([val, label, desc]) => (
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              ...
              <button onClick={() => { setShowInvite(false); setForm({ email: '', name: '', role: 'admin', teamId: '' }); }} style={{ flex: 1, background: '#242424', color: '#9ca3af', border: '1px solid #2a2a2a', borderRadius: '10px', padding: '13px', fontSize: '14px', cursor: 'pointer' }}>
                Cancel
              </button>
            </div>
          </div>
