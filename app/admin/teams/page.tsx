...
export default function TeamsPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [tr, ur] = await Promise.all([fetch('/api/teams').then(r=>r.json()), fetch('/api/admin-users').then(r=>r.json())]);
      setTeams(tr.teams||[]); setUsers(ur.users||[]);
    } catch {}
    setLoading(false);
  };
  const saveTeam = async () => {
    ...
    setSaving(true);
    await fetch('/api/teams', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ action: editId?'update':'create', team: form, teamId: editId }) });
    setToast(editId ? 'Team updated!' : 'Team "'+form.name+'" created!');
    setForm({...EMPTY}); setEditId(null); setShowForm(false);
    await load(); setSaving(false); setTimeout(()=>setToast(''),3000);
  };
    ...
    <div style={{ background:'#131313', minHeight:'100vh', padding:'28px', maxWidth:'1000px' }}>
      ...
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:'28px' }}>
        <div>
          ...
          <p style={{ color:'#6b7280', fontSize:'13px', margin:0 }}>Group users into teams — each team sees only their own CRM, inventory, purchases and sales data</p>
        </div>
        <button onClick={()=>{ setShowForm(true); setEditId(null); setForm({...EMPTY}); }} style={{ background:'#7b1c2e', color:'#fff', border:'none', borderRadius:'9px', padding:'10px 20px', fontSize:'13px', fontWeight:600, cursor:'pointer' }}>+ New Team</button>
      </div>
      <div style={{ background:'rgba(59,130,246,0.07)', border:'1px solid rgba(59,130,246,0.18)', borderRadius:'12px', padding:'14px 18px', marginBottom:'24px', display:'flex', gap:'12px' }}>
        ...
        <div style={{ color:'#4b5563', fontSize:'13px', lineHeight:1.6 }}>
          <strong style={{ color:'#93c5fd' }}>How it works:</strong> Admin and Viewer users only see data for their assigned team. The <strong style={{ color:'#9ca3af' }}>Owner</strong> (master PIN) sees all teams. Assign users below or from the <a href="/admin/users" style={{ color:'#c0394f', textDecoration:'none' }}>Users page</a>.
        </div>
      </div>
      ...
      : teams.length === 0 ? (
        <div style={{ background:'#1a1a1a', border:'1px solid #2a2a2a', borderRadius:'14px', padding:'60px', textAlign:'center' }}>
          ...
          <div style={{ color:'#4b5563', fontSize:'13px', marginBottom:'22px' }}>Create your first team to start scoping data per user group.</div>
          <button onClick={()=>{ setShowForm(true); setForm({...EMPTY}); }} style={{ background:'#7b1c2e', color:'#fff', border:'none', borderRadius:'9px', padding:'11px 22px', fontSize:'14px', fontWeight:600, cursor:'pointer' }}>+ Create First Team</button>
        </div>
      ) : (
        <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
          {teams.map((team, idx) => {
            ...
            return (
              <div key={team.teamId} style={{ background:'#1a1a1a', border:'1px solid '+(isOpen?c+'55':'#2a2a2a'), borderRadius:'14px', overflow:'hidden', transition:'border-color 0.2s' }}>
                <div style={{ padding:'16px 20px', display:'flex', alignItems:'center', gap:'14px' }}>
                  ...
                  <div style={{ display:'flex', alignItems:'center', gap:'8px', flexShrink:0 }}>
                    <span style={{ background:c+'18', color:c, border:'1px solid '+c+'30', borderRadius:'20px', padding:'3px 10px', fontSize:'11px', fontWeight:600 }}>{members.length} member{members.length!==1?'s':''}</span>
                    <button onClick={()=>setExpanded(isOpen?null:team.teamId)} style={{ background:'#242424', border:'1px solid #2a2a2a', borderRadius:'7px', color:'#9ca3af', fontSize:'12px', padding:'5px 12px', cursor:'pointer' }}>{isOpen?'↑ Collapse':'↓ Members'}</button>
                    <button onClick={()=>{ setForm({name:team.name,description:team.description||''}); setEditId(team.teamId); setShowForm(true); }} style={{ background:'#242424', border:'1px solid #2a2a2a', borderRadius:'7px', color:'#9ca3af', fontSize:'12px', padding:'5px 12px', cursor:'pointer' }}>Edit</button>
                        ...
                        <tbody>
                          {members.map(u=>(
                            <tr key={u.email} onMouseOver={e=>e.currentTarget.style.background='#1a1a1a'} onMouseOut={e=>e.currentTarget.style.background='transparent'}>
                              <td style={td}><div style={{display:'flex',alignItems:'center',gap:'10px'}}><div style={{width:'28px',height:'28px',borderRadius:'50%',background:'rgba(123,28,46,0.3)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'11px',fontWeight:700,color:'#f87171',flexShrink:0}}>{(u.name||u.email).charAt(0).toUpperCase()}</div><span style={{color:'#fff',fontWeight:600,fontSize:'13px'}}>{u.name||'—'}</span></div></td>
      ...
      {showForm && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.8)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:50 }}>
          <div style={{ background:'#1a1a1a', border:'1px solid #2a2a2a', borderRadius:'18px', padding:'32px', width:'100%', maxWidth:'420px', boxShadow:'0 32px 64px rgba(0,0,0,0.5)' }}>
            ...
            <p style={{ color:'#6b7280', fontSize:'13px', margin:'0 0 24px' }}>{editId?'Update this team's details.':'Teams isolate data — users only see their team's records.'}</p>
            <div style={{ marginBottom:'16px' }}>
              <label style={lbl}>Team Name *</label>
              <input type="text" placeholder="e.g. Sales Team, East Region" value={form.name} onChange={e=>setForm(p=>({...p,name:e.target.value}))} onKeyDown={e=>e.key==='Enter'&&saveTeam()} style={inp} autoFocus />
            </div>
            <div style={{ marginBottom:'28px' }}>
              <label style={lbl}>Description <span style={{ color:'#374151', textTransform:'none', letterSpacing:0 }}>(optional)</span></label>
            ...
            <div style={{ display:'flex', gap:'10px' }}>
              <button onClick={saveTeam} disabled={saving||!form.name.trim()} style={{ flex:1, background:form.name.trim()&&!saving?'#7b1c2e':'#2d0e18', color:form.name.trim()&&!saving?'#fff':'#5a2030', border:'none', borderRadius:'10px', padding:'13px', fontSize:'14px', fontWeight:700, cursor:form.name.trim()&&!saving?'pointer':'not-allowed' }}>{saving?'Saving...':editId?'Save Changes':'Create Team'}</button>
